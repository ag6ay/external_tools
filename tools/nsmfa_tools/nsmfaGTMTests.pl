...
################################################################################
##                                                                            ##
##  nsmfaGTMTests.pl - NSMFA GTM smoke tests used to validate NSMFA           ##
...
if (1)
{
   ...
   #Send Request
   ($httpResCode, $httpResponseHeader, $httpResponseBody, $httpResponse) = httpRequest("$vars{$env}{'url'}/notificationService/v2/fis/$vars{$env}{'fiid'}/products/IB/notificationApps/MFA/fiCustomers/$vars{$env}{'authid'}/destinations", \%httpHeaderHash, $agent, $readTimeOut, $httpContentType, 'GET', $httpBody, $outboundProxy);
