...
################################################################################
##                                                                            ##
##  p2pGTMTests.pl - P2P GTM smoke tests used to validate P2P is functional   ##
##                   against Global Traffic Manager.                          ##
##                                                                            ##
##                   Created by: David Schwab                                 ##
##                   Last updated: DS - 04/29/2014 Ver. 1.0                   ##
##                                                                            ##
##                                                                            ##
################################################################################
################################################################################
...
$vars{'qa_qdc'}{'url'}                           = 'https://p2pservices-qal.banking.intuit.net';
...
$vars{'qal1_dca_dcb'}{'url'}                     = 'http://p2p.qal1.diginsite.net';

#################################################################
#Define prod_qdc_lvdc Variables below
#################################################################
$vars{'prod_qdc_lvdc'}{'guid'}                   = 'b33264360010347a51b7a2d226cbe700';
$vars{'prod_qdc_lvdc'}{'fiid'}                   = '01425';
$vars{'prod_qdc_lvdc'}{'url'}                    = 'https://p2pservices.banking.intuit.net';
$vars{'prod_qdc_lvdc'}{'auth'}                   = '7bd60d7552c0450a98fde14b80d5cb1f';

#################################################################
#Define prd1_dca_dcb Variables below
#################################################################
$vars{'prd1_dca_dcb'}{'guid'}                    = 'b33264360010347a51b7a2d226cbe700';
$vars{'prd1_dca_dcb'}{'fiid'}                    = '01425';
$vars{'prd1_dca_dcb'}{'url'}                     = 'http://p2p.prd1.diginsite.net';
...
if (0)
{
   ...
   #Send Request
   ($httpResCode, $httpResponseHeader, $httpResponseBody, $httpResponse) = httpRequest("$vars{$env}{'url'}/p2p/status", \%httpHeaderHash, $agent, $readTimeOut, $httpContentType, 'GET', $httpBody, $outboundProxy);
...
{
   #Set Test Name
   $testName = 'p2p-getFinancialInstitution-v1';
   ...
   #Send Request
   ($httpResCode, $httpResponseHeader, $httpResponseBody, $httpResponse) = httpRequest("$vars{$env}{'url'}/p2p/v1/p2pfis/$vars{$env}{'fiid'}", \%httpHeaderHash, $agent, $readTimeOut, $httpContentType, 'GET', $httpBody, $outboundProxy);
...
if (1)
{
   ...
   #Send Request
   ($httpResCode, $httpResponseHeader, $httpResponseBody, $httpResponse) = httpRequest("$vars{$env}{'url'}/p2p/v1/p2pfis/$vars{$env}{'fiid'}/users/$vars{$env}{'guid'}", \%httpHeaderHash, $agent, $readTimeOut, $httpContentType, 'GET', $httpBody, $outboundProxy);
...
if (1)
{
   ...
   #Send Request
   ($httpResCode, $httpResponseHeader, $httpResponseBody, $httpResponse) = httpRequest("$vars{$env}{'url'}/p2p/v1/p2pfis/$vars{$env}{'fiid'}/users/$vars{$env}{'guid'}/accounts", \%httpHeaderHash, $agent, $readTimeOut, $httpContentType, 'GET', $httpBody, $outboundProxy);
...
{
   #Set Test Name
   $testName = 'p2p-getDataChanges-v1';
   ...
   #Send Request
   ($httpResCode, $httpResponseHeader, $httpResponseBody, $httpResponse) = httpRequest("$vars{$env}{'url'}/p2p/v1/p2pfis/$vars{$env}{'fiid'}/users/$vars{$env}{'guid'}/datachanges", \%httpHeaderHash, $agent, $readTimeOut, $httpContentType, 'GET', $httpBody, $outboundProxy);
