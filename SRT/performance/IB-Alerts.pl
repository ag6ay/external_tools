...
while ( $tickticker <=  $tickticker_sto)
{
	...
	if ($check_clear eq "true"){
		while ($item_cnt <= $check_clear_itm_cnt){
			 print "0,ISSUEITEM,ALERT,$t_initial,$t_final,,,,,,,,,908,,2758248117,,1,,$tickticker,,$email_add,SRT Alert Remove,Check Clear Notification Message~nBreak~nDouble Break~n~n~mYou should not see user values here:(~n~a~u)~mFooter Text,0,2,1234\n";
			$item_cnt++;
		}
		$item_cnt = "1";
	}
	...
	if ($maturity_date eq "true"){
		while ($item_cnt <= $maturity_date_itm_cnt){
			print "0,SWEEPITEM,ALERT,$t_initial,$t_final,,,,IMATDT,IMATDT,-3d,LTE,TODAY,,465855,465855,16,16,,$tickticker,DATE,$email_add,SRT Alert Remove,Maturity Date Notification Message,0,10,1234\n";
			$item_cnt++;
		}
		$item_cnt = "1";
	}
	...
	if ($payment_due eq "true"){
		while ($item_cnt <= $payment_due_itm_cnt){
			print "0,SWEEPITEM,ALERT,$t_initial,$t_final,,,,LNXTPT,LNXTPT,-4d,LTE,TODAY,,365179,365179,64,64,,$tickticker,DATE,$email_add,SRT Alert Remove,Payment Due Notification Message~nBreak~nDouble Break~n~nAmount Due (LPAY):~p~nPayment Due Date (LNXTPT):~d~mYou should not see user values here:(~n~a~u)~mFooter Text,0,10,1234\n";
			$item_cnt++;
		}
		$item_cnt = "1";
	}
